...
4. **On next unlock**: if `migration_status == "pending"`, resume the recorded migration mode. For `re_encrypt`, the supplied passphrase is first verified against the preserved old check token (`vault_old_check`); the batch re-runs only if it matches the old key. After a genuine passphrase change the supplied passphrase is the new one, so the match fails and the resume only clears the markers — this safely converges the case where the batch had completed and only the finalize write was lost, but it cannot recover a crash that interrupted the batch itself (see caveat).
...
> **Re-encrypt is recoverable only after the batch completes.** Vault keys are derived from passphrase + salt alone, with no wrapped data-encryption key, so re-encryption needs the old and new keys at once. Once `migration_start` installs the new check token, only the new passphrase can unlock and the old key can no longer be derived. A crash **during** the `re_encrypt` batch therefore leaves the already-rotated records under the new key and the rest stranded under the old key with no way to decrypt them; the resume detects the mismatch and clears the markers without touching the stranded records (it must not re-run the batch, which would double-encrypt the rotated ones). Recovering the stranded records requires restoring that user's data from backup.
