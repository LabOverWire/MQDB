...
**Change passphrase recovery.** Before re-encryption begins, the old salt and old check token are persisted on the identity record alongside the new salt and new check token. The new check token takes effect immediately, so after `migration_start` the only passphrase that can unlock is the new one. On the next unlock the server verifies the supplied passphrase against the preserved old check token and re-runs the batch only when it matches the old key. After a genuine passphrase change the supplied passphrase is the new one, so the match fails; the only time it succeeds is a same-passphrase salt rotation where the old and new passphrases are identical.

This recovers exactly one failure: the batch had already finished and only the `migration_done` write was lost. Every record is then already under the new key, the resume recognises the mismatch, skips the batch (re-running it would double-encrypt the rotated records), and clears the markers. A crash **during** the batch is *not* recoverable: keys are derived from passphrase and salt alone (there is no wrapped data-encryption key), so finishing the rotation would need both the old and new keys at once, but after `migration_start` only the new passphrase can unlock and the old key can no longer be derived. Records the batch already rotated stay readable under the new key; any record not yet rotated stays under the old key and can no longer be decrypted. The guard prevents this half-finished state from being made worse — the previous behaviour wrapped the already-rotated records a second time and destroyed them permanently. Operators should treat a crash during `re_encrypt` as a restore-from-backup event for the affected user. When the resume completes, the old salt and old check token are cleared.
