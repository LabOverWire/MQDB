...
impl<T: ClusterTransport> NodeController<T> {
    ...
    pub async fn start_scatter_list_query(
        ...
        filters: Vec<mqdb_core::Filter>,
        sorts: Vec<mqdb_core::SortOrder>,
        projection: Option<Vec<String>>,
        pagination: Option<mqdb_core::Pagination>,
        sender: Option<&str>,
    ) -> bool {
    ...
    pub async fn handle_scatter_list_response(
            ...
            && let Some(completed) = self.pending_scatter_requests.remove(&request_id)
        {
            ...
            if let Some(ref vault_sender) = completed.vault_sender
                && mqdb_vault::transform::is_vault_eligible(
                    &completed.entity,
                    &self.ownership,
                )
    ...
    fn sort_scatter_results(results: &mut [serde_json::Value], sorts: &[mqdb_core::SortOrder]) {
        if sorts.is_empty() {
            return;
        }
        ...
        results.sort_by(|a, b| {
            ...
            for order in sorts {
                ...
                let cmp = match order.direction {
                    mqdb_core::SortDirection::Asc => cmp,
                    mqdb_core::SortDirection::Desc => cmp.reverse(),
                };
