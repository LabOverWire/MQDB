...
use mqdb_core::relationship::RelationshipRegistry;
use mqdb_core::schema::SchemaRegistry;
...
impl Database {
    ...
    pub async fn open<P: AsRef<std::path::Path>>(path: P) -> mqdb_core::error::Result<Self> {
    ...
    /// # Errors
    /// Returns an error if the database cannot be opened.
    pub async fn open_with_config(config: DatabaseConfig) -> mqdb_core::error::Result<Self> {
    ...
    pub async fn open_with_backend(
        backend: Arc<dyn mqdb_core::storage::StorageBackend>,
    ...
    async fn init_with_storage(
        ...
        if let Some(names) = persisted {
            ...
            for key in keys {
                if let Ok((entity, _)) = mqdb_core::keys::decode_data_key(&key)
    ...
    pub fn event_receiver(
        &self,
    ) -> tokio::sync::broadcast::Receiver<mqdb_core::events::ChangeEvent> {
    ...
    pub async fn all_schemas(&self) -> Vec<mqdb_core::schema::Schema> {
    ...
    pub async fn all_constraints(
        &self,
    ) -> std::collections::HashMap<String, Vec<mqdb_core::constraint::Constraint>> {
