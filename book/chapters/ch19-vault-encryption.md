...
**Encryption** operates at the field level, not the record level, and recurses through the full JSON structure. Given a record like `{"id": "rec-1", "name": "Alice", "profile": {"city": "Paris", "age": 30}, "tags": ["personal", "draft"]}`, the vault encrypts every string leaf value at any depth — `"Alice"`, `"Paris"`, `"personal"`, and `"draft"` are each encrypted independently, while the number `30`, the object structure, and the array structure pass through unchanged. Non-string values (numbers, booleans, null) remain cleartext at all depths — AES-256-GCM operates on byte sequences, and encrypting a number would require serializing it to bytes, encrypting, base64-encoding, and storing as a string, losing type information and queryability in the process.
...
The batch encryption is the expensive part. For a user who owns 500 records across three entities, enable reads each record, recursively encrypts all string leaf values, and writes it back as an update. This is why enable acquires an exclusive fence (described in Section 19.4) — concurrent reads during batch encryption would see a mix of encrypted and plaintext records.
...
6. Recursively encrypts all string leaf values (skipping `id` and the owner field at the top level, and `_`-prefixed keys at all depths)
...
**Structural encryption preserves shape at the cost of metadata leakage.** Recursively encrypting string leaf values while preserving JSON structure means an observer can see the full shape of a record: key names, nesting depth, array lengths, and the types of non-string values. A record like `{"profile": {"city": "base64(...)", "age": 30}, "tags": ["base64(...)", "base64(...)"]}` reveals that there is a nested object with a string and a number, and an array with two encrypted strings — without knowing any plaintext. Record-level encryption (encrypting the entire JSON blob as a single ciphertext) would hide this metadata but would prevent the database from operating on any field without decryption. The tradeoff is acceptable for MQDB's use case: the database needs to read `id` and owner fields for routing and access control, and numeric/boolean fields must remain queryable for filters and sorting.
