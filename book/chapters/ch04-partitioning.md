...
The fix was to tie bootstrap to leader election. When a node wins a Raft election and discovers it is the only node in the cluster, it triggers the rebalancer immediately, which proposes all 256 partition assignments in a single Raft batch. The accept loop still starts before Raft bootstraps — the broker accepts TCP connections while the partition map is empty. But any write that arrives during that gap hits the third branch of write-or-forward: no primary exists, so the operation returns a 503 error. The health check endpoint (`$DB/_health`) reports `ready: false` until all 256 partitions have a primary and a Raft leader exists. MQDB's own CLI tools and benchmarks poll the health check before sending operations, so in practice the bootstrap window is invisible to well-behaved clients.

This is a two-layer defense. The write-or-forward logic rejects operations during the gap with an explicit error code. The health check advertises readiness so that clients can wait without polling the data path. Neither layer alone is sufficient — without the 503 rejection, a client that skips the health check would see silent drops; without the health check, every client would need to handle and retry 503 errors during startup.
...
When a second node joins, the rebalancer runs. Since partitions are already initialized, it uses incremental rebalancing rather than computing from scratch. The incremental algorithm cannot redistribute primaries in the first cycle because Phase 1 only promotes existing replicas — and Node 2 holds no replicas yet. Instead, the first cycle's Phase 2 adds Node 2 as a replica for approximately 128 partitions, and Phase 3 further balances replica distribution. After a periodic balance check (30 seconds), the second cycle runs: now Node 2 is a replica for ~128 partitions, so Phase 1 can promote it to primary for those partitions while demoting Node 1 to replica. The result after two cycles is ~128 primaries and ~128 replicas per node. This multi-cycle convergence is fundamental to the incremental algorithm — it never assigns a primary role to a node that lacks replica data for the partition.
