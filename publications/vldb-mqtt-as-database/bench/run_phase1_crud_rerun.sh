...
if ! guard_aws_dir; then
    echo "run_phase1_crud_rerun.sh: guard_aws_dir refused the RESULTS_DIR" >&2
    exit 1
fi
...
main() {
    if [ "${1:-}" = "--dry-run" ]; then
        echo "run_phase1_crud_rerun.sh: --dry-run OK (RESULTS_DIR=$RESULTS_DIR)"
        exit 0
    fi
    ...
    echo ""
    echo "============================================"
    echo "  CRUD RE-RUN (persistent subscriptions     "
    echo "  with correlation data in db_sync harness)  "
