...
- **Unique seal fails closed when there is no established voter set.** When a node had no established unique-voter set — a fresh cluster before the leader founds it, or a node between a process restart and adopting the gossiped set — the majority/seal denominator fell back to the full partition-map membership, and a promoting primary could seal over a membership larger than the established voter set a reservation was a majority of, miss its holders, and oversell. Two code paths did this: the async seal, and the synchronous `seal_or_queue_unique` self-seal reached when an unsynced (empty) local map made the node look single-node. Both now fail closed — a multi-node cluster with no established voter set (empty voters, or an empty/unsynced map) waits until the set is founded or adopted (a tick / a heartbeat) instead of falling back. A GENUINE single-node cluster (the replicated map names it the sole member) still self-serves — it is trivially its own voter set. Modeled in `specs/ClusterUniqueReconfigV7.tla` (the full-membership fallback oversells; failing closed is safe).
