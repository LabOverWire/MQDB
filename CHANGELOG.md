...
- Presence is a **hint**, not a ledger: messages may be dropped under load (the queue is bounded) and can be reordered across a flap, so `ts` is authoritative. Correctness still rests on the TTL backstop plus a returning holder re-asserting its hold; see the invariant model in `specs/AbandonedHoldReclaim.tla`.
- Because messages are retained, the broker keeps one retained message per distinct `client_id`, overwritten in place on each transition. That is bounded by the client population, but a deployment that uses a fresh random `client_id` for every session will accumulate retained entries — prefer stable client ids when enabling presence.
...
- **Resurrection guard for a revoked grant.** Because email resolution is asynchronous, an `unshare` issued while a share is still resolving could otherwise be undone when the resolution completes and writes the grant. An `unshare` now invalidates any in-flight resolution for the same `(resource, grantee)` on the node holding it, and a completing resolution that was invalidated writes nothing. Invalidation is authorized (the share's issuer, an admin, or the resource owner when co-located) so a non-owner cannot cancel someone else's in-flight share. The race and the guard are model-checked in `specs/ShareResolveRace.tla` (the unguarded model violates a no-resurrection invariant; the guarded model holds).
...
- Known follow-ups tracked in #122: identity resolution currently runs before the primary-side owner/admin check, so a non-owner's `share` can trigger one `_identity_links` scatter before the write is rejected (bounded, authenticated, identity-mode only) — the durable fix is to authorize on the resource primary before resolving; a transient window with no alive peers writes an under-resolved pending grant (healed by the pending-grant sweep); and the scatter `request_id` reuses the existing wall-clock scheme shared with list scatters.
...
- **Grantee emails are no longer stored in plaintext in `_shares` (identity mode).** `grantee_key` holds a blind index of the lowercased email (matching the identity subsystem's `email_hash` pattern), and the email itself is retained only encrypted in `grantee_email`, decrypted on the fly when the owner lists a resource's grants. Nothing persists the grantee email in the clear. `_shares` change events therefore carry the opaque `grantee_key` and the encrypted `grantee_email`; the plaintext email appears only in the owner's `shares` listing response.
...
- **Unique reconciler no longer stalls the node.** The periodic cluster unique reconciler cloned every record of every unique-constrained entity and held the controller write lock across the whole scan and its sends, stalling all DB ops on the node every 10s. It now runs in two passes: a cheap key-gather (`collect_unique_reconcile_keys`, no JSON parse) over the store under a brief read lock, then per 64-key chunk it parses the records (`project_unique_reconcile_work`, read lock) and applies the reasserts (write lock). Records are never cloned (new `DbDataStore::for_each_record`) and the dominant JSON-parse cost is chunked out of the long lock hold, so DB traffic interleaves. Splitting the lock is made safe by an apply-time re-check that a record still owns the value, so a record deleted mid-cycle cannot resurrect a committed claim.
